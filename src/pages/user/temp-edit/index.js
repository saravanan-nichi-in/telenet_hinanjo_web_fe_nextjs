...
import { setRegisterData, setOriginalData,setPlaceId, reset } from "@/redux/userTempEdit";
import { setPlaceId as setPlace } from "@/redux/tempRegister";
...
export default function Admission() {
  ...
  const [placeId,setPlaceIds] = useState("")
  ...
  const fetchOrginalData = (originalData) => {
    setPlaceIds(originalData[0]?originalData[0].place_id:"")
    if (originalData[0]?.place_id) {
    ...
    if (!Object.keys(regReducer.originalData).length > 0 && originalData && (originalData).length > 0) {
  ...
  const fetchMasterQuestion = (originalData) => {
    ...
    getMasterQuestionnaireList(payload, (res) => {
      try{
      if (res) {
        const sourceData = regReducer?.originalData?.questions?.length > 0
        ? regReducer.originalData
        : originalData[0]?.questions?.length > 0
        ? originalData[0]
        : null;
        const updatedList = res.data.list.map(item => {
          if (sourceData) {
          const matchingQuestion = sourceData.questions.find(question => question.id === item.id);
  ...
  const getEvacueesFamilyDetail = (response) => {
    if (response) {
      if (response.data.data.length > 0) {
        ...
        if(!regReducer?.originalData?.questions?.length > 0)
        {
  ...
  function convertToOriginalFormat(convertedData) {
    ...
    const originalData = {
      data: [
        {
          place_id:convertedData.data[0].place_id,
                      ...
                      <div className="grid">
                        ...
                        <div className="mb-2  col-12 xl:col-12">
                          ...
                          <div className="body_table">
                            ...
                            {values?.address ? values.address : ""}{values?.address2 ? values.address2 : ""}
                          </div>
                        </div>
