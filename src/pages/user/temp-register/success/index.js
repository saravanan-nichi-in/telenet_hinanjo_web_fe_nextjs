...
const RegisterSuccess = () => {
  ...
  useEffect(() => {
    ...
    if (
      localStorage.getItem("deletedFromStaff") == "true" ||
      localStorage.getItem("tempDataDeleted") == "true" ||
      !regReducer.successData?.data?.familyCode
    ) {
      ...
      TempRegisterServices.isRegistered(payload, (res) => {
        if (res) {
          ...
          if (
            ...
            !default_place_id.includes(parseInt(place_id))
          ) {
  ...
  const handleDownload = async () => {
    ...
    html2canvas(element, { scale: 2, useCORS: true, allowTaint: true }).then(
      (canvas) => {
        ...
        <div className="card flex flex-column h-full align-items-center justify-content-center">
          <div className="mdScreenMaxWidth xlScreenMaxWidth">
            ...
            {isTemp && (
              <div
                id="capture"
                className="mb-3 text-center block grid col-12 successButtonText"
              >
                <Button
                  buttonProps={{
                    type: "button",
                ...
                {showDelete && (
                  <Button
                    buttonProps={{
                      type: "button",
                      buttonClass:
                        "w-full delete-button-user h-5rem border-radius-5rem mt-3 border-2",
