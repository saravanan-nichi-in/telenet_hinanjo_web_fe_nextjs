...
import { InputIcon, InputNumber } from "../input";
import { FileUpload } from "../upload";

const StockSignupModal = (props) => {
    const { parentMainClass,modalClass, position, contentClass,value,options,onChange,placeholder,selectParentClass,onClickTop,OnClickAddition} = props
    ...
    return (
        <div className={`${parentMainClass}`}>
            ...
            <Dialog className={`${modalClass}`} draggable={false} position={"top" || position} header={header} footer={footer} visible={visible} onHide={() => setVisible(false)} style={{ width: '600px', padding: "10px" }} >
